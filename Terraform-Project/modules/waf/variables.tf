...
variable "managed_rule_groups" {
  ...
  type = list(object({
    name            = string   # 예: AWSManagedRulesCommonRuleSet
    vendor_name     = string   # 보통 AWS
    priority        = number
    override_action = string   # none 또는 count
  }))
  default = []
}

variable "rate_limit" {
  description = "Rate-based rule limit (5분 기준). 0이면 비활성"
  type        = number
  default     = 0
}
...
variable "custom_rules" {
  ...
  type = list(object({
    name     = string
    priority = number
    action   = string  # "block" 또는 "allow"
    statement = object({
      patterns = list(string)
      field_to_match = string  # "uri", "query_string", "body", "all_query_arguments"
    })
  }))
  default = []
}
