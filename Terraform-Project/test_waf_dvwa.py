...
"""
DVWA WAF 테스트 스크립트
SQL Injection 공격 차단 확인

WAF 규칙:
- AWSManagedRulesSQLiRuleSet: SQL Injection 공격 차단
"""
...
def test_1_sql_injection():
    ...
    AWSManagedRulesSQLiRuleSet이 차단해야 함
    """
    print("\n[테스트 1] SQL Injection 공격 시작...")
    
    # 다양한 SQL Injection 페이로드
    sql_payloads = [
        "' OR '1'='1",
        "' OR 1=1--",
        "' UNION SELECT NULL--",
    ...
    for payload in sql_payloads:
        try:
            # DVWA 로그인 페이지에 SQL Injection 시도
            params = {
                'id': payload,
                'username': payload,
                'password': payload,
            ...
            # 403 Forbidden 또는 405 Method Not Allowed = 차단
